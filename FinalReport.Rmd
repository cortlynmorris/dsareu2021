...
```{r contrib-circum-injury, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r driver-age-injury, echo=FALSE, fig.width = 10}
...
```{r driver-age, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r passenger-age-injury, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r passenger-age, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r vehicle-injury, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r weather-injury, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r road-feat-injury, echo=FALSE, fig.width = 10}
crashes %>%
...
```{r 7-day-ma-ts, echo=FALSE, warning=FALSE, fig.width = 10}
crashes_ts %>%
...
```{r monthly-ts, echo=FALSE, message=FALSE, fig.width=10}
forecast::autoplot(crashes_mts4) +
  ggtitle("Time Series Plot for Frequency of Daily Crashes (With Pandemic Data)") + xlab("Date") + ylab("Monthly Crashes") + 
...
forecast::autoplot(crashes_mts4.noncovid) +
  ggtitle("Time Series Plot for Frequency of Daily Crashes (Without Pandemic Data)") + xlab("Date") + ylab("Monthly Crashes") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8), 
                     labels = c("Jan 2015", "Jan 2016", "Jan 2017", 
                                "Jan 2018", "Jan 2019", "Jan 2020", 
                                "Jan 2021", "Jan 2022")) + 
...
```{r stationarity-trans-daily-covid, fig.width = 10}
...
```{r stat-trans-daily-covid-plot, fig.width = 10}
...
The plot above, shows the data before the stationary transformation and after the first difference was taken (stationary data)
...
The Exponential smoothing state space model with Box-Cox transformation, ARMA errors, Trend and Seasonal components (TBATS model) was chosen because it is known to perform well at forecasting with long daily time series. 
...
The autoregressive integrated moving average (ARIMA) model was chosen because it is known to perform well at forecasting with long daily time series. 
...
```{r stationarity-trans-daily-noncovid, fig.width = 10}
...
Here our daily nonpandemic data needs one first difference and we do this with the diff() function. Using the diff() function, we take the first difference and then we check the acf of the data to make sure that it does not take too many lags to hit 0 indicating that the first difference transformed our data to be stationary.

```{r stat-trans-daily-noncovid-plot, fig.width = 10}
...
```{r stationarity-trans-monthly-covid, fig.width = 10}
...
Here our monthly pandemic data needs one first difference and we do this with the diff() function. Using the diff() function, we take the first difference and then we check the acf of the data to make sure that it does not take too many lags to hit 0 indicating that the first difference transformed our data to be stationary.

```{r stat-trans-monthly-covid-plot, fig.width = 10}
...
From the time series plot above... 
...
```{r stationarity-trans-monthly-noncovid, fig.width = 10}
...
Here our monthly nonpandemic data needs one seasonal difference and we do this with the diff() function and adding the argument "lag=12" to indicate a seasonal difference. Using the diff() function, we take the seasonal difference and then we check the acf of the data to make sure that it does not take too many lags to hit 0 indicating that the first difference transformed our data to be stationary.

```{r stat-trans-monthly-noncovid-plot, fig.width = 10}
...
The Seasonal Autoregressive Integrated Moving Average (SARIMA) model was chosen because it is able to automatically handle our monthly nonpandemic data that needs to be seasonally differenced. It was also chosen because it has been shown to create accurate forecasts for seasonally adjusted data. 
...
The Seasonal and Trend Decomposition using Loess Forecasting Model (STLF) was chosen because it is able to automatically handle our monthly nonpandemic data that needs to be seasonally differenced. It was also chosen because it has been shown to create accurate forecasts for seasonally adjusted data. 
...
```{r tbats-ms-stationary-plot, echo=FALSE, fig.width = 10}
#Stationary 
...
From the time series forecast above... 
...
From the root mean square error, we can see that the TBATS model performs much better than the ARIMA and the Combination model. The model with the small root mean square error (smallest error in general), is said to perform better. 
...
From the root mean square error, we can see that the three models perform similarly, however, the ARIMA model does slightly better than the other two models. 
...
```{r tbats-ms-stationary-plot-monthly, echo=FALSE, fig.width = 10}
#Stationary 
autoplot(training_TBATS2) + 
...
From the root mean square error, we can see that all of the models did not perform very well, most likely due to the unforeseen large dip in car crashes during the pandemic that very much effects future forecasts. However, out of the four HoltWinters has the lowest RMSE and performs slightly better than the other models. 
...
Despite the very similar plots, from the RMSE (Root Mean Squared Error) we can see that the combination performs slightly better than the other two models. 
...
(Insert Injury vs. Noninjury Frequency Bar chart) 

Due to this imbalance, we decided to test a variety of balancing methods to compare the accuracy of each of these using the logistic regression model.
...
Salas-Eljatib, Christian, et al. “A Study on the Effects of Unbalanced Data When Fitting Logistic Regression Models in Ecology.” Ecological Indicators, Elsevier, 7 Nov. 2017, www.sciencedirect.com/science/article/abs/pii/S1470160X17306659. 
...
```{r contrib-circum-injury-code, eval=FALSE}
crashes %>%
  filter(Injury != "NA", Injury != "Unknown", ContributingCircumstance1 != "NA", 
         ContributingCircumstance1 != "NaN", 
         ContributingCircumstance1 != "Unable to determine", 
...
```{r driver-age-injury-code, eval=FALSE}
# Comparing Injury to Driver Age
crashes %>%
  filter(Injury != "NA", Injury != "Unknown", Injury != "", PersonType == "Driver", Age != "NA",
         Age != "", Age != "Unknown", Age >= 5) %>%
...
```{r passenger-age-injury-code, eval=FALSE}
crashes %>%
  filter(Injury != "NA", Injury != "Unknown", Injury != "", PersonType == "Passenger", Age != "NA",
         Age != "", Age != "Unknown") %>%
  group_by(Injury) %>%
...
```{r passenger-age-code, eval=FALSE}
crashes %>%
  filter(PersonType == "Passenger", Age != "NA",
         Age != "", Age != "Unknown") %>%
  mutate(age = case_when(
...
```{r vehicle-injury-code, eval=FALSE}
crashes %>%
  filter(Injury != "NA", Injury != "Unknown", VehicleType != "NA", 
         VehicleType != "NaN", VehicleType != "Unknown", VehicleType != "") %>%
  group_by(Injury) %>%
...
```{r weather-injury-code, eval=FALSE}
crashes %>%
  filter(Injury != "NA", Injury != "Unknown", WeatherCondition1 != "NA", 
         WeatherCondition1 != "NaN", WeatherCondition1 != "") %>%
  group_by(Injury) %>%
...
```{r road-feat-injury-code, eval=FALSE}
crashes %>%
  filter(Injury != "NA", Injury != "Unknown", RoadFeature != "NA", 
         RoadFeature != "NaN", RoadFeature != "") %>%
  group_by(Injury) %>%
...
```{r load-library-word-cloud-code, message=FALSE, eval=FALSE}
#install.packages('wordcloud2')
library(wordcloud2)
```
...
```{r load-library-interactive-map-code, message=FALSE, eval=FALSE}
library(ggplot2)
#install.packages("ggmap")
library(ggmap)
library(RColorBrewer)
...
```{r nc-interactive-map-code, message=FALSE, warning=FALSE, eval=FALSE}
map_bounds <- c(-78.98, 35.51, -78.24, 36.06)
...
```{r wake-interactive-map-code, message=FALSE, warning=FALSE, eval=FALSE}
crashes_filter <- crashes %>%
  filter(LocationLatitude != "0", LocationLongitude != "0",
         LocationLatitude != "NA", LocationLatitude != "",
         LocationLongitude != "NA", LocationLongitude != "",
...
```{r load-library-ts-code,eval=FALSE, message=FALSE}
library(zoo)
```
...
```{r 7-day-ma-ts-code, eval=FALSE, warning=FALSE}
crashes_ts %>%
  filter(as.Date(Date) >= "2015/01/01") %>%
  ggplot(aes(x = as.Date(Date), y = count)) + 
  geom_line() +
...
```{r tbats-stationary-code, eval=FALSE, message=FALSE, include=FALSE}
#Stationary
...
```{r tbats-stationary-plot-code, eval=FALSE, message=FALSE}
#Stationary
autoplot(crashests2) + 
  autolayer(Yhat, series="Point Forecasts") + 
  #autolayer(Yhat_ci_upper) + 
...
```{r tbats-ms-stationary-code, eval=FALSE, message=FALSE, include=FALSE}
#Stationary
...
```{r combination-code, eval=FALSE, message=FALSE}
crashes_ts = crashes %>%
  filter(as.Date(crash_date) >= "2015/01/01" & as.Date(crash_date) <= "2021/05/31") %>%
  separate(crash_date, 
           into = c("Date", "Hour"), sep = 11) %>%
...
```{r tbats-noncovid-stationary-code, eval=FALSE, message=FALSE, include=FALSE}
#Stationary
...
```{r tbats-noncovid-stationary-plot-code, eval=FALSE, message=FALSE}
#Stationary
...
```{r combination-noncovid-code, eval=FALSE, message=FALSE}
crashes_ts.noncovid = crashes %>%
  filter(as.Date(crash_date) >= "2015/01/01" & as.Date(crash_date) <= "2020/03/01") %>%
  separate(crash_date, 
           into = c("Date", "Hour"), sep = 11) %>%
...
```{r tbats-stationary-monthly-code, eval=FALSE, message=FALSE, include=FALSE}
#Stationary
...
```{r tbats-stationary-plot-monthly-code, eval=FALSE, message=FALSE}
#Stationary
autoplot(crashes_mts4) + 
  autolayer(Yhat_monthly, series="Point Forecasts") + 
...
```{r tbats-ms-stationary-monthly-code, eval=FALSE, message=FALSE, include=FALSE}
#Stationary
dif_y2_monthly <- msts(dif_crashes_mts4, seasonal.periods=12)
...
```{r arima-monthly-code, eval=FALSE, message=FALSE}
fit_arima_monthly <- auto.arima(crashes_mts4)
...
```{r ms-arima-monthly-code, eval=FALSE, message=FALSE}
#Stationary
training_arima_monthly <- subset(crashes_mts4, end=length(crashes_mts4)-5)
...
```{r monthly-HW-fcast-filter-forecast-ts-stationary-code, eval=FALSE, message=FALSE}
#Stationary 
...
```{r ms-HW-monthly-forecast-ts-stationary-code, eval=FALSE, message=FALSE}
#Stationary  
dif_training_HW_monthly <- subset(dif_crashes_mts4, end=length(dif_crashes_mts4)-5)
...
```{r combination-monthly-code,eval=FALSE, message=FALSE}
train_monthly <- window(crashes_mts4, end=c(5,12))
...
```{r sarima-library-code, eval=FALSE, message=FALSE}
#install.packages("astsa")
library(astsa)
```

```{r sarima-code, eval=FALSE, message=FALSE, include=FALSE}
fc_sarima <- sarima.for(crashes_mts4.noncovid, n.ahead=23, 1,0,0,0,1,1,12) 
```

```{r sarima-plot-code, eval=FALSE, message=FALSE}
fc_sarima <- ts(fc_sarima$pred, start=c(6,3),frequency=12)
...
```{r ms-sarima-code, eval=FALSE, message=FALSE, include=FALSE}
training_sarima <- subset(crashes_mts4.noncovid, end=length(crashes_mts4.noncovid)-14)
...
```{r ms-sarima-plot-code, eval=FALSE, message=FALSE}
autoplot(training_sarima) + 
  autolayer(crashes.train_sarima, series="Point Forecasts") + 
  autolayer(test_sarima, series = "Test Set") + 
...
```{r stlf-code, eval=FALSE, message=FALSE}
q <- crashes_mts4.noncovid %>%
  stlf(lambda = 0, h = 23, level=c(80,95)) 
...
```{r noncovid-ms-STLF-forecast-ts-code, eval=FALSE, message=FALSE}
training_STLF_noncovid_monthly <- subset(crashes_mts4.noncovid, end=length(crashes_mts4.noncovid)-14)
...
```{r combination-monthly-noncovid-code, eval=FALSE, message=FALSE, include=FALSE}
train_monthly.noncovid <- window(crashes_mts4.noncovid, end=c(5,12))
...
```{r combination-monthly-noncovid-plot-code, eval=FALSE, message=FALSE}
#STLF Model 
STLF <- stlf(train_monthly.noncovid, lambda = 0, h = 14) 
