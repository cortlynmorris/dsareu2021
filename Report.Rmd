...
```{r location-direction, echo=FALSE}
crashes %>% 
...
```{r HW-fcast-box-decomp, echo=FALSE}
...
fcast <- forecast::forecast(covid, h=214, level=c(65,80))
```

```{r rename-HW-fcast-column, echo=FALSE, include=FALSE}
...
```{r HW_fcast-filter-forecast-ts, echo=FALSE, message=FALSE}
...
m <- crashests2 %>%
  stlf(lambda = 0, h = 214, level=c(65,80)) 
...
```{r HW-noncovid-fcast-decomp, echo=FALSE}
...
fcast.noncovid <- forecast(noncovid, h=671, level=c(65,80))
```

```{r rename-noncovid-HW-fcast, echo=FALSE, include=FALSE}
...
```{r noncovid-HW-fcast-filter-forecast-ts, echo=FALSE}
...
p <- crashests.noncovid %>%
  stlf(lambda = 0, h = 671, level = c(65, 80)) 
...
```{r ms-HW-forecast-ts, echo=FALSE, message=FALSE}
...
crashes.train_HW %>%
  forecast::forecast(h=151, level=c(65,80)) %>%
...
fcast.ms.HW.daily.covid <- summary(forecast::forecast(crashes.train_HW, h=152, level=c(65,80)))

rownames(fcast.ms.HW.daily.covid) <- seq(as.Date("2021/01/01"), 
                                                   as.Date("2021/06/01"), "day")
```

```{r ms-HW-fcast-filter-decomp-sum, echo=FALSE, message=FALSE}
fcast.ms.HW.daily.covid
...
crashes.train_STLF <- stlf(training_STLF, lambda = 0, h = 152, level=c(65,80))
crashes.train_STLF %>%
...
```{r noncovid-ms-HW-forecast-ts, echo=FALSE, message=FALSE}
...
crashes.train_HW_noncovid %>%
  forecast::forecast(h=426, level=c(65,80)) %>%
...
```{r rename-noncovid-ms-HW-fcast, echo, include=FALSE}
fcast.ms.HW.daily.noncovid <- summary(forecast::forecast(crashes.train_HW_noncovid, h=426, level=c(65,80)))

rownames(fcast.ms.HW.daily.noncovid) <- seq(as.Date("2019/01/01"), 
                                         as.Date("2020/03/01"), "day")
```

```{r noncovid-ms-HW-fcast-filter-decomp-sum, echo=FALSE, message=FALSE}
fcast.ms.HW.daily.noncovid
...
```{r noncovid-ms-STLF-forecast-ts, echo=FALSE}
...
crashes.train_STLF_noncovid <- stlf(training_STLF_noncovid, lambda = 0, h = 426, level=c(65,80))
...
```{r noncovid-ms-HW-fcast-filter-decomp-sum, echo=FALSE, warning=FALSE}
fcast.ms.STLF.daily.noncovid
...
fcast <- forecast::forecast(fit.crashes, h=8, level=c(65,80))
```
...
fcast.noncovid <- forecast::forecast(fit.crashes.noncovid, h=23, level=c(65,80))
```
...
fcast.covid.monthly <- forecast::forecast(covid.monthly, h=8, level=c(65,80))
```
...
fcast.noncovid.monthly <- forecast::forecast(noncovid.monthly, h=23, level=c(65,80))
```
...
t <- crashes_mts4 %>%
  stlf(lambda = 0, h = 8, level=c(65,80)) 
...
q <- crashes_mts4.noncovid %>%
  stlf(lambda = 0, h = 23, level=c(65,80)) 
...
crashes.train_HW_monthly %>%
  forecast::forecast(h=5, level=c(65,80)) %>%
...
fcast.ms.HW.monthly.covid <- summary(forecast::forecast(crashes.train_HW_monthly, h=5, level=c(65,80)))
...
crashes.train_STLF_monthly <- stlf(training_STLF_monthly, lambda = 0, h = 5, level=c(65,80))
...
crashes.train_HW_noncovid_monthly %>%
  forecast::forecast(h=14, level=c(65,80)) %>%
...
crashes.train_STLF_noncovid_monthly <- stlf(training_STLF_noncovid_monthly, lambda = 0, h = 14, level=c(65,80))
...
fcast.ms.STLF.monthly.noncovid
...
```{r regression-2015-2019-ts, echo=FALSE, message=FALSE}
...
```{r 2015-2019-HW-fcast-decomp, echo=FALSE}
...
fcast.2019 <- forecast::forecast(twentynineteen, h=61, level=c(65,80))
```
...
```{r 2015-2019-HW-fcast-filter-forecast-ts, echo=FALSE}
...
```{r 2015-2019-STLF-fit-forecast-ts, echo=FALSE}
crashests.2019 %>% mstl() %>%
  autoplot() + 
  ggtitle("Box Jenkins Decomposition") + 
  xlab("Date") + ylab("Count")
...
p <- crashests.2019 %>%
  stlf(lambda = 0, h = 61, level=c(65,80)) 
...
```{r 2015-2019-fcast-filter, echo=FALSE}
twenty_nineteen_daily_forecast_values_STLF
```

```{r 2015-2019-monthly-ts, echo=FALSE, message=FALSE}
...
```{r 2015-2019-lin-monthly-forecast-ts, echo=FALSE, message=FALSE}
fit.crashes.2019 <- tslm(crashes_mts4.2019 ~ trend + season)

summary(fit.crashes.2019)

fcast.2019 <- forecast::forecast(fit.crashes.2019, h=3, level=c(65,80))
...
```{r 2015-2019-HW-fit-fcast-true-forecast-ts, echo=FALSE, message=FALSE}
...
```{r 2015-2019-STLF-monthly-fit-fcast-forecast-ts, echo=FALSE, message=FALSE}
...
t <- crashes_mts4.2019 %>%
  stlf(lambda = 0, h = 3, level=c(65,80)) 
