...
crashests2
dif_crashests2<-diff(crashests2)
z0<-cumsum(c(crashests2[1], dif_fc_tbats_covid2$fitted))
...
str(dif_fc_tbats_covid2)

lapply(dif_fc_tbats_covid2, function(x) as.numeric(x$mean, na.rm = TRUE))
...
#dif_crashests2 <- diff(crashests2)
dYhat <- crashests2[length(crashests2)]
...
```{r}
#Stationary
rownames(dif_daily_forecast_values_covid_TBATS2) <- seq(as.Date("2021/06/01"), as.Date("2021/12/31"), "day")

head(dif_daily_forecast_values_covid_TBATS2)
```

```{r}
#Stationary
autoplot(dif_fc_tbats_covid2) + 
  ggtitle("TBATS Forecasting Model for Daily Car Crashes (With Pandemic Data)") +
  xlab("Date") + ylab("Crashes")
```
...
```{r noncovid-STLF-decomp-forecast-ts-stationary, echo=FALSE}
#Stationary
...
```{r rename-noncovid-STLF-fcast-stationary, echo=FALSE, include=FALSE}
#Stationary
daily_forecast_values_noncovid_STLF <- summary(p)
...
#Stationary
y2.noncovid <- msts(dif_crashests.noncovid, seasonal.periods=c(7,365.25))
...
```{r tbats-noncovid-plot-stationary, echo=FALSE, message=FALSE}
#Stationary
head(daily_forecast_values_noncovid_TBATS2)
