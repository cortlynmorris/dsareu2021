...
```{r stationarity-trans-daily-covid, echo=FALSE, include=FALSE}
...
```{r stat-trans-daily-covid-plot, echo=FALSE}
...
```{r stationarity-trans-daily-noncovid, echo=FALSE, include=FALSE}
...
```{r stat-trans-daily-noncovid-plot, echo=FALSE}
...
#                                                   as.Date("2021/12/31"), "day")
rownames(daily_forecast_values_covid_TBATS2) <- seq(as.Date("2021/06/01"), as.Date("2021/12/31"), "day")
```
...
```{r noncovid-STLF-decomp-forecast-ts-stationary, echo=FALSE, eval=FALSE}
#Stationary (does not work)
dif_crashests.noncovid %>% mstl() %>%
  autoplot() + 
  ggtitle("Time Series Decomposition (Without Pandemic Data)")

dif_p <- dif_crashests.noncovid %>%
  stlf(lambda = 0, h = 671, level = c(80, 95)) 
...
autoplot(crashests.noncovid) + 
  autolayer(true_noncovid_crashes, alpha=0.7) +
  autolayer(Yhat_noncovid, alpha=0.6) + 
  ggtitle("TBATS Forecasting Model for Daily Car Crashes (Stationary Without Pandemic Data)") +
  xlab("Date") + ylab("Crashes") + 
  theme(title = element_text(size = 9), legend.position = "bottom")
```

```{r ms-HW-forecast-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE
...
```{r rename-ms-HW-fcast, echo=FALSE, eval=FALSE}
#DONT USE
...
```{r ms-HW-fcast-filter-decomp-sum, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE
...
crashes.train_TBATS2 %>%
  ...
  ggtitle("Multi-Step TBATS Daily Forecasts of Crashes (With Pandemic Data)") + 
  xlab("Date") + ylab("Crashes") + 
  autolayer(test_TBATS2, alpha=0.75, series="test set") + 
  theme(legend.position = "bottom")
```
...
autoplot(training_TBATS2) + 
  autolayer(Yhat_ms_TBATS) + 
  autolayer(test_TBATS2, series="test_TBATS2", alpha = 0.7) + 
...
```{r noncovid-ms-HW-forecast-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE
...
```{r rename-noncovid-ms-HW-fcast, echo=FALSE, eval=FALSE}
#DONT USE
...
```{r noncovid-ms-HW-fcast-filter-decomp-sum, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE
...
```{r tbats-ms-noncovid-plots, echo=FALSE, message=FALSE, warning=FALSE}
#Nonstationary
...
autoplot(fcast) +
  ggtitle("Forecasts of Monthly Car Crashes Using Linear Model (With Pandemic Data)") + xlab("Date") + ylab("Monthly Crashes") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8), 
                     labels = c("Jan 2015", "Jan 2016", "Jan 2017", "Jan 2018", 
                                "Jan 2019", "Jan 2020", "Jan 2021", "Jan 2022")) + 
  theme(axis.text.x = element_text(angle = 90))
...
autoplot(fcast.noncovid) +
  ...
  xlab("Date") + ylab("Monthly Crashes") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7, 8), 
...
```{r stationarity-trans-monthly-covid, echo=FALSE, include=FALSE}
...
```{r stat-trans-monthly-covid-plot, echo=FALSE}
...
```{r noncovid-monthly-HW-fcast-decomp-stationary, echo=FALSE}
#Stationary 
dif.noncovid.monthly <- HoltWinters(season_dif_crashes_mts4.noncovid)

dif.fcast.noncovid.monthly <- forecast::forecast(dif.noncovid.monthly, h=23, level=c(80,95))

dYhat.HW.noncovid <- dif.fcast.noncovid.monthly$mean

Yhat.HW.noncovid <- cumsum(c(crashes_mts4.noncovid[length(crashes_mts4.noncovid)],
                             dYhat.HW.noncovid))

Yhat.HW.noncovid <- ts(Yhat.HW.noncovid, start = c(6, 2), frequency=12)
```

```{r noncovid-monthly-fcast-filter-forecast-ts-stationary, echo=FALSE, message=FALSE}
#Stationary 
autoplot(crashes_mts4.noncovid) +
  autolayer(Yhat.HW.noncovid) + 
  ggtitle("Forecasts of Monthly Car Crashes Using HoltWinters (Stationary Without Pandemic Data)") +
  xlab("Date") + ylab("Crashes") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7, 8), 
                     labels = c("Jan 2015", "Jan 2016", "Jan 2017", "Jan 2018", 
                                "Jan 2019", "Jan 2020", "Jan 2021", "Jan 2022")) + 
...
```{r monthly-STLF-forecast-ts-decomp-stationary, echo=FALSE, message=FALSE}
#Stationary
dif_crashes_mts4 %>% mstl() %>%
  autoplot() + 
  ggtitle("Time Series Decomposition") + 
  xlab("Date") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8), 
...
dif_t <- dif_crashes_mts4 %>% #warning: NaNs produced
  stlf(lambda = 0, h = 8, level=c(80,95)) 

dYhat_stlf_monthly <- dif_t$mean

Yhat_stlf_monthly <- cumsum(c(crashes_mts4[length(crashes_mts4)], dYhat_stlf_monthly))

Yhat_stlf_monthly <- ts(Yhat_stlf_monthly, start=c(7, 6), frequency=12)

autoplot(crashes_mts4) +
  autolayer(Yhat_stlf_monthly) + 
  ggtitle("SLTF Model for Monthly Car Crashes (With Stationary Pandemic Data)") +
  xlab("Date") + ylab("Crashes") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8), 
                     labels = c("Jan 2015", "Jan 2016", "Jan 2017", "Jan 2018", 
                                "Jan 2019", "Jan 2020", "Jan 2021", "Jan 2022")) + 
...
```{r noncovid-monthly-STLF-forecast-ts-decomp-stationary, echo=FALSE, message=FALSE}
#Stationary 
season_dif_crashes_mts4.noncovid %>% mstl() %>%
  autoplot() + 
  ggtitle("Time Series Decomposition") + 
  xlab("Date") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8), 
...
dif_q <- season_dif_crashes_mts4.noncovid %>% #warning: NaNs produce
  stlf(lambda = 0, h = 23, level=c(80,95)) 

dYhat_stlf_monthly_noncovid <- dif_q$mean

Yhat_stlf_monthly_noncovid <- cumsum(c(crashes_mts4.noncovid[length(crashes_mts4.noncovid)],dYhat_stlf_monthly_noncovid))

Yhat_stlf_monthly_noncovid <- ts(Yhat_stlf_monthly_noncovid, start = c(6, 2), frequency=12)

autoplot(crashes_mts4.noncovid) + 
  autolayer(Yhat_stlf_monthly_noncovid) + 
...
```{r noncovid-ms-HW-monthly-forecast-ts-stationary, echo=FALSE, message=FALSE}
#Stationary 
dif_training_HW_noncovid_monthly <- subset(season_dif_crashes_mts4.noncovid, 
                                       end=length(season_dif_crashes_mts4.noncovid)-14)

dif_test_HW_noncovid_monthly <- subset(season_dif_crashes_mts4.noncovid, 
                                   start=length(season_dif_crashes_mts4.noncovid)-13)

dif_crashes.train_HW_noncovid_monthly <- HoltWinters(dif_training_HW_noncovid_monthly)

dif_fc_HW_noncovid <- forecast::forecast(dif_crashes.train_HW_noncovid_monthly, h = 14)

dYhat_ms_HW_noncovid <- dif_fc_HW_noncovid$mean 

Yhat_ms_HW_noncovid <- cumsum(c(crashes_mts4.noncovid[length(crashes_mts4.noncovid)], 
                                dYhat_ms_HW_noncovid))

Yhat_ms_HW_noncovid <- ts(Yhat_ms_HW_noncovid, start = c(5, 1), frequency = 12)
...
autoplot(training_HW_noncovid_monthly) + 
  autolayer(Yhat_ms_HW_noncovid) + 
  autolayer(test_HW_noncovid_monthly) + 
  ggtitle("Multi-Step HW Monthly Forecasts of Crashes (Stationary Without Pandemic Data)") + 
  xlab("Date") + ylab("Crashes") + 
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7), 
                     labels = c("Jan 2015", "Jan 2016", "Jan 2017", "Jan 2018", 
                                "Jan 2019", "Jan 2020", "Jan 2021")) + 
...
```{r ets-MAM-covid-stationary, echo=FALSE, message=FALSE, eval=FALSE}
#Stationary (Does not work)
##Working with ets() monthly covid MAM model 
dif.fit.ets.monthly.covid <- ets(y=dif_crashes_mts4, model="MAM")
#ERORR: Error in ets(y = dif_crashes_mts4, model = "MAM") : 
...
```{r 2015-2019-ts, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r regression-2015-2019-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-HW-fcast-decomp, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r rename-2015-2019-HW-fcast-true-val-crash, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-HW-fcast-filter-forecast-ts, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-STLF-fit-forecast-ts, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
crashests.2019 %>% mstl() %>%
  autoplot() + 
...
```{r rename-2015-2019-STLF-fcast-true-val-crash, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-fcast-filter, echo=FALSE, eval=FALSE}
#DONT USE (REPEAT)
twenty_nineteen_daily_forecast_values_STLF
```

```{r 2015-2019-monthly-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-lin-monthly-forecast-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-HW-fit-fcast-true-forecast-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE (REPEAT)
...
```{r 2015-2019-STLF-monthly-fit-fcast-forecast-ts, echo=FALSE, message=FALSE, eval=FALSE}
#DONT USE (REPEAT)
